...
type UIChatSearchTeamHits struct {
	Hits             []khulnasoft1.TeamSearchItem `codec:"hits" json:"hits"`
...
func (o UIChatSearchTeamHits) DeepCopy() UIChatSearchTeamHits {
	return UIChatSearchTeamHits{
		Hits: (func(x []khulnasoft1.TeamSearchItem) []khulnasoft1.TeamSearchItem {
			if x == nil {
				return nil
			}
			ret := make([]khulnasoft1.TeamSearchItem, len(x))
			for i, v := range x {
				vCopy := v.DeepCopy()
				ret[i] = vCopy
			}
...
type UIChatSearchBotHits struct {
	Hits             []khulnasoft1.FeaturedBot `codec:"hits" json:"hits"`
	SuggestedMatches bool                   `codec:"suggestedMatches" json:"suggestedMatches"`
}

func (o UIChatSearchBotHits) DeepCopy() UIChatSearchBotHits {
	return UIChatSearchBotHits{
		Hits: (func(x []khulnasoft1.FeaturedBot) []khulnasoft1.FeaturedBot {
			if x == nil {
				return nil
			}
			ret := make([]khulnasoft1.FeaturedBot, len(x))
			for i, v := range x {
				vCopy := v.DeepCopy()
				ret[i] = vCopy
			}
...
type UIBotCommandsUpdateSettings struct {
	Settings map[string]khulnasoft1.TeamBotSettings `codec:"settings" json:"settings"`
}

func (o UIBotCommandsUpdateSettings) DeepCopy() UIBotCommandsUpdateSettings {
	return UIBotCommandsUpdateSettings{
		Settings: (func(x map[string]khulnasoft1.TeamBotSettings) map[string]khulnasoft1.TeamBotSettings {
			if x == nil {
				return nil
			}
			ret := make(map[string]khulnasoft1.TeamBotSettings, len(x))
			for k, v := range x {
				kCopy := k
				vCopy := v.DeepCopy()
				ret[kCopy] = vCopy
