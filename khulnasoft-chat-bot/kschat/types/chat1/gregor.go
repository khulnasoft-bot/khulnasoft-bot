...
type UnreadUpdate struct {
	...
	UnreadNotifyingMessages map[khulnasoft1.DeviceType]int `codec:"unreadNotifyingMessages" json:"unreadNotifyingMessages"`
...
func (o UnreadUpdate) DeepCopy() UnreadUpdate {
	return UnreadUpdate{
		...
		UnreadNotifyingMessages: (func(x map[khulnasoft1.DeviceType]int) map[khulnasoft1.DeviceType]int {
			if x == nil {
				return nil
			}
			ret := make(map[khulnasoft1.DeviceType]int, len(x))
			for k, v := range x {
				kCopy := k.DeepCopy()
				vCopy := v
				ret[kCopy] = vCopy
