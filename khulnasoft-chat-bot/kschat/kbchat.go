...
type RunOptions struct {
	...
	// Number of processes to spin up to connect to the khulnasoft service
	NumPipes int
}

func (r RunOptions) Location() string {
	if r.KhulnasoftLocation == "" {
		return "khulnasoft"
	}
	return r.KhulnasoftLocation
}
...
// Start fires up the Khulnasoft JSON API in stdin/stdout mode
...
// API is the main object used for communicating with the Khulnasoft JSON API
type API struct {
	sync.Mutex
	*DebugOutput
...
func (a *API) getUsername(runOpts RunOptions) (username string, err error) {
	...
	go func() {
		...
		var status khulnasoft1.CurrentStatus
		...
		if status.LoggedIn && status.User != nil {
			...
			doneCh <- fmt.Errorf("unable to authenticate to khulnasoft service: logged in: %v user: %+v", status.LoggedIn, status.User)
		}
		// Cleanup the command
	...
	case <-time.After(a.Timeout):
		return "", errors.New("unable to run Khulnasoft command")
	}
...
func (a *API) startPipes() (err error) {
	...
	// Startup NumPipes processes to the khulnasoft chat api
...
func (a *API) LogSend(feedback string) error {
	feedback = "go-khulnasoft-chat-bot log send\n" +
