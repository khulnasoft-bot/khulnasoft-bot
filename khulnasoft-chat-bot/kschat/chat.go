...
func (a *API) JoinChannel(teamName string, channelName string) (chat1.EmptyRes, error) {
	...
	if err != nil {
		return empty, fmt.Errorf("failed to parse output from khulnasoft team api: %v", err)
...
func (a *API) ListMembers(channel chat1.ChatChannel) (khulnasoft1.TeamMembersDetails, error) {
...
func (a *API) ListMembersByConvID(conversationID chat1.ConvIDStr) (khulnasoft1.TeamMembersDetails, error) {
	arg := newListMembersArg(listMembersOptions{
...
func (a *API) listMembers(arg listMembersArg) (res khulnasoft1.TeamMembersDetails, err error) {
	bArg, err := json.Marshal(arg)
	if err != nil {
		return res, err
	}
