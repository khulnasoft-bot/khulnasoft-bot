...
type khulnasoftTestConfig struct {
	Khulnasoft string
	Bots    map[string]*OneshotOptions
	Teams   map[string]chat1.ChatChannel
}

func readAndParseTestConfig(t *testing.T) (config khulnasoftTestConfig) {
...
func testBotSetup(t *testing.T, botName string) (bot *API, dir string) {
	...
	kbLocation := whichKhulnasoft(t)
	if len(config.Khulnasoft) > 0 {
		kbLocation = config.Khulnasoft
	}
	dir = randomTempDir(t)
	kbTmpLocation := prepWorkingDir(t, dir, kbLocation)
	bot, err := Start(RunOptions{KhulnasoftLocation: kbTmpLocation, HomeDir: dir, Oneshot: config.Bots[botName], StartService: true})
	require.NoError(t, err)
	return bot, dir
}
