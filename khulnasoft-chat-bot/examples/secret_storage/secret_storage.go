...
Khulnasoft has added an encrypted key-value store intended to support
...
entryValue. Namespaces and entryKeys are in cleartext, and the Khulnasoft client
service will encrypt and sign the entryValue on the way in (as well as
decrypt and verify on the way out) so khulnasoft servers cannot see it or
forge it.
...
Khulnasoft) can know about the names of all the cool tools you have; you can do
...
// plaintext of entryKeys from Khulnasoft servers.  It does so by HMACing
...
//
// This approach does not hide memory access patterns. Also, Khulnasoft servers
...
func NewSecretKeyKVStoreAPI(api kschat.KVStoreAPI) *SecretKeyKVStoreAPI {
	config := SecretKeyKVStoreAPIConfig{
		secretName:            "_secret",
...
func (sc *SecretKeyKVStoreAPI) loadSecret(teamName string, namespace string) ([]byte, error) {
	...
	if err != nil {
		if e, ok := err.(kschat.Error); !ok || e.Code != kschat.RevisionErrorCode {
			// unexpected error
			return nil, err
		}
...
func (sc *SecretKeyKVStoreAPI) PutEntry(teamName *string, namespace string, entryKey string, entryValue string) (result khulnasoft1.KVPutResult, err error) {
	return sc.PutEntryWithRevision(teamName, namespace, entryKey, entryValue, 0)
}

func (sc *SecretKeyKVStoreAPI) PutEntryWithRevision(teamName *string, namespace string, entryKey string, entryValue string, revision int) (result khulnasoft1.KVPutResult, err error) {
	if teamName == nil {
...
func (sc *SecretKeyKVStoreAPI) PutEntryWithRevisionAndTeam(teamName string, namespace string, entryKey string, entryValue string, revision int) (result khulnasoft1.KVPutResult, err error) {
...
func (sc *SecretKeyKVStoreAPI) DeleteEntry(teamName *string, namespace string, entryKey string) (result khulnasoft1.KVDeleteEntryResult, err error) {
	return sc.DeleteEntryWithRevision(teamName, namespace, entryKey, 0)
}

func (sc *SecretKeyKVStoreAPI) DeleteEntryWithRevision(teamName *string, namespace string, entryKey string, revision int) (result khulnasoft1.KVDeleteEntryResult, err error) {
	if teamName == nil {
		return result, fmt.Errorf("teamName must be defined for SecretKeyKVStoreAPI methods")
	}
...
func (sc *SecretKeyKVStoreAPI) DeleteEntryWithRevisionAndTeam(teamName string, namespace string, entryKey string, revision int) (result khulnasoft1.KVDeleteEntryResult, err error) {
	hmacEntryKey, err := sc.obfuscateEntryKey(teamName, namespace, entryKey)
	if err != nil {
		return result, err
	}
...
func (sc *SecretKeyKVStoreAPI) GetEntry(teamName *string, namespace string, entryKey string) (result khulnasoft1.KVGetResult, err error) {
	if teamName == nil {
		return result, fmt.Errorf("teamName must be defined for SecretKeyKVStoreAPI methods")
	}
...
func (sc *SecretKeyKVStoreAPI) GetEntryWithTeam(teamName string, namespace string, entryKey string) (result khulnasoft1.KVGetResult, err error) {
	hmacEntryKey, err := sc.obfuscateEntryKey(teamName, namespace, entryKey)
	if err != nil {
		return result, err
	}
...
func (sc *SecretKeyKVStoreAPI) ListNamespaces(teamName *string) (khulnasoft1.KVListNamespaceResult, error) {
	return sc.ListNamespaces(teamName)
}

func (sc *SecretKeyKVStoreAPI) ListEntryKeys(teamName *string, namespace string) (result khulnasoft1.KVListEntryResult, err error) {
...
func NewRentalBotClient(api kschat.KVStoreAPI, teamName string, namespace string) *RentalBotClient {
	r := RentalBotClient{api: api, team: teamName, namespace: namespace}
	return &r
}

func (r *RentalBotClient) Lookup(tool string) (khulnasoft1.KVGetResult, error) {
...
func (r *RentalBotClient) Add(tool string) (ok bool, result khulnasoft1.KVGetResult, err error) {
...
func (r *RentalBotClient) Remove(tool string) (ok bool, result khulnasoft1.KVGetResult, err error) {
	result, err = r.Lookup(tool)
	if err != nil {
		return false, result, err // api call failed
	} else if result.EntryValue == nil {
...
func (r *RentalBotClient) Reserve(username string, tool string, day string) (ok bool, result khulnasoft1.KVGetResult, err error) {
...
// Returns (whether action is successful, most recent get result if applicable, error)
func (r *RentalBotClient) Unreserve(username string, tool string, day string) (ok bool, result khulnasoft1.KVGetResult, err error) {
	var reservations map[string]string
	result, err = r.Lookup(tool)
	if err != nil {
		return false, result, err // api call failed
